...
function applyTableSetting(attrs, callback) {
  var renderSetting = attrs.renderSetting;
  var toolbars = attrs.toolbars;
  var fieldToolbars = [];
  renderSetting.cols[0].forEach(function (col) {
    if (col.toolbar && col.toolbars) {
      col.toolbars.map(function (bar) {
        return fieldToolbars.push(bar);
      });
    }
  });
  layui.use(["form", "table", "laytpl", "laydate"], function () {
    ...
    var layRenderSetting = Object.assign(defaultRenderSetting, renderSetting);
    var tableIns = table.render(layRenderSetting);
    var defaultOpenSetting = {
      title: "",
      type: 2,
      shadeClose: false,
      maxmin: false,
    ...
    table.on("toolbar(Lay)", function (obj) {
      ...
      var eventData = toolbar.onEvent(null, tableIns); // 存在数据时返回的时layer.open的设置
    ...
    table.on("tool(Lay)", function (obj) {
      ...
      var toolbar = null;
      fieldToolbars.forEach(function (bar) {
        if (bar.event === layEvent) {
          toolbar = bar;
        }
      });
      ...
      var eventData = toolbar.onEvent(data, tableIns); // 存在数据时返回的时layer.open的设置
